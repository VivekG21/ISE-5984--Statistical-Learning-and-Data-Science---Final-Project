...
# Creating a training set based on undersampling the raw data set

undersample_train <- downSample(x = data[, -ncol(data)],
                                y = data$Class)
table(undersample_train$Class)
...
# KNN Classification on Under-sampled Training Set Data

# Choose maximum number of clusters k
k <- 15
...
set.seed(9560)
smote_train <- SMOTE(Class ~ ., data=data)

table(smote_train$Class)


model <- train(Class ~.,data= undersample_train,method="knn",
